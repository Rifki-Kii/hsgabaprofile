<div>
    ...
    <div class="flex justify-between items-center mb-4">
        <p class="text-slate-500 text-xs">Atur daftar kegiatan ekstrakurikuler minat dan bakat beserta deskripsi dan dokumentasi kegiatannya.</p>
        ...
        @forelse ($ekskuls as $ekskul)
            <div class="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden flex flex-col group">
                ...
                <div class="relative h-28 bg-slate-900 overflow-hidden flex items-center justify-center">
                    @if ($ekskul->image_path)
                        @if(str_starts_with($ekskul->image_path, 'http'))
                            <img src="{{ $ekskul->image_path }}" class="w-full h-full object-cover opacity-60 group-hover:scale-105 transition-transform duration-500">
                        @else
                            <img src="{{ asset($ekskul->image_path) }}" class="w-full h-full object-cover opacity-60 group-hover:scale-105 transition-transform duration-500">
                        @endif
                    @else
                        <div class="w-full h-full bg-slate-800 opacity-60 flex items-center justify-center text-slate-400">
                ...
                <div class="p-4 flex-1 flex flex-col justify-between">
                    <div>
                        <h4 class="font-bold text-slate-800 text-xs mb-1 truncate">{{ $ekskul->name }}</h4>
                        <p class="text-[11px] text-slate-500 leading-relaxed truncate-2-lines mb-3" title="{{ $ekskul->description }}">{{ $ekskul->description }}</p>
                    </div>
                    ...
                    <div class="flex justify-between items-center border-t border-slate-100 pt-2.5">
                        ...
                        <div class="flex items-center gap-1">
                            <button wire:click="edit({{ $ekskul->id }})" 
                            ...
                            <button onclick="confirm('Apakah Anda yakin ingin menghapus ekstrakurikuler ini?') || event.stopImmediatePropagation()"
                                    wire:click="delete({{ $ekskul->id }})" 
        ...
        @empty
            <div class="col-span-4 bg-white rounded-xl border border-slate-200 p-8 text-center text-slate-400 shadow-xs">
                <div class="flex flex-col items-center gap-2">
                    <i class="fa-solid fa-star text-3xl text-slate-300"></i>
                    <span class="text-xs">Belum ada data ekstrakurikuler yang disimpan.</span>
                </div>
            </div>
        @endforelse
    </div>
            ...
            <div class="bg-white rounded-2xl w-full max-w-lg shadow-xl overflow-hidden border border-slate-100">
                ...
                <div class="px-5 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                    <h3 class="text-xs font-bold text-slate-800 uppercase tracking-wider">
                        {{ $isEditMode ? 'Ubah Ekstrakurikuler' : 'Tambah Ekstrakurikuler Baru' }}
                    </h3>
                ...
                <form wire:submit.prevent="{{ $isEditMode ? 'update' : 'store' }}">
                    <div class="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
                        <!-- Image Upload -->
                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Foto Kegiatan (Optional)</label>
                        ...
                        <div class="grid grid-cols-3 gap-3">
                            <div class="col-span-2">
                                <label for="name" class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Nama Ekskul</label>
                                <input type="text" id="name" wire:model="name" 
                                       class="w-full border border-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" placeholder="e.g. Berkuda">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label for="category" class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Kategori / Badge</label>
                                <input type="text" id="category" wire:model="category" 
                                       class="w-full border border-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" placeholder="e.g. Sunnah Rasul">
                                @error('category') <span class="text-red-500 text-[10px] mt-0.5 block">{{ $message }}</span> @enderror
                            </div>
                        ...
                        <div>
                            <label for="description" class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Manfaat / Deskripsi Kegiatan</label>
                            <textarea id="description" wire:model="description" rows="3"
                                      class="w-full border border-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" placeholder="Jelaskan secara ringkas tentang manfaat atau pencapaian ekskul ini..."></textarea>
                    ...
                    <div class="px-5 py-3 border-t border-slate-100 bg-slate-50 flex justify-end gap-2">
                        ...
                        <button type="submit" 
                            ...
                            {{ $isEditMode ? 'Simpan Perubahan' : 'Tambah Ekskul' }}
                        </button>
                    </div>
                </form>
