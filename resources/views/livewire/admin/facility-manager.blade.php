<div>
    ...
    <div class="flex justify-between items-center mb-4">
        <p class="text-slate-500 text-xs">Kelola fasilitas pendukung untuk pembelajaran siswa yang tampil pada slide bagian Fasilitas.</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @forelse ($facilities as $fac)
            <div class="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden flex flex-col group">
                ...
                <div class="relative h-36 bg-slate-900 overflow-hidden">
                    @if(str_starts_with($fac->image_path, 'http'))
                        <img src="{{ $fac->image_path }}" class="w-full h-full object-cover opacity-80 group-hover:scale-105 transition-transform duration-500">
                    @else
                        <img src="{{ asset($fac->image_path) }}" class="w-full h-full object-cover opacity-80 group-hover:scale-105 transition-transform duration-500">
                    @endif
                    <div class="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-900/20 to-transparent"></div>
                    
                    <!-- Icon badge overlay -->
                    <div class="absolute top-3 right-3 w-8 h-8 bg-blue-600/80 text-white rounded-full flex items-center justify-center backdrop-blur-sm">
                        <i class="{{ $fac->icon }} text-xs"></i>
                    </div>
                ...
                <div class="p-4 flex-1 flex flex-col justify-between">
                    <div>
                        <h4 class="font-bold text-slate-800 text-xs mb-1 truncate">{{ $fac->title }}</h4>
                        <p class="text-[11px] text-slate-500 leading-relaxed mb-3 truncate-2-lines">{{ $fac->description }}</p>
                    </div>

                    <div class="flex items-center justify-end gap-1.5 border-t border-slate-100 pt-2.5">
                        <button wire:click="edit({{ $fac->id }})" 
        ...
        @empty
            <div class="col-span-3 bg-white rounded-xl border border-slate-200 p-8 text-center text-slate-400 shadow-xs">
                <div class="flex flex-col items-center gap-2">
                    <i class="fa-solid fa-building text-3xl text-slate-300"></i>
                    <span class="text-xs">Belum ada data fasilitas yang ditambahkan.</span>
                </div>
            </div>
        @endforelse
    </div>
            ...
            <div class="bg-white rounded-2xl w-full max-w-lg shadow-xl overflow-hidden border border-slate-100">
                ...
                <form wire:submit.prevent="{{ $isEditMode ? 'update' : 'store' }}">
                    <div class="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
                        <!-- Image Upload -->
                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Foto Fasilitas</label>
                        ...
                        <div class="grid grid-cols-3 gap-3">
                            <div class="col-span-2">
                                <label for="title" class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Nama Fasilitas</label>
                                <input type="text" id="title" wire:model="title" 
                                       class="w-full border border-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" placeholder="e.g. Kolam Renang">
                            ...
                            <div>
                                ...
                                <input type="text" id="icon" wire:model="icon" 
                                       class="w-full border border-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors text-center font-mono" placeholder="fas fa-swimmer">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div class="flex items-end">
                                <span class="text-[9px] text-slate-400 mb-2 leading-tight">
                                    💡 Masukkan kode ikon FontAwesome seperti <code>fas fa-mosque</code> atau <code>fas fa-book</code>.
                                </span>
                            </div>
                        </div>

                        <div>
                            <label for="description" class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Deskripsi / Penjelasan Singkat</label>
                            <textarea id="description" wire:model="description" rows="3"
                                      class="w-full border border-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" placeholder="Keterangan mengenai fasilitas ini..."></textarea>
